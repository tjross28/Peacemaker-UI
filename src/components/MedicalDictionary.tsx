...
import { medicalDictionary, categoryDescriptions } from '../utils/medicalDictionary';

interface MedicalDictionaryProps {
  onBack: () => void;
}

type Category = 'all' | 'measurement' | 'condition' | 'device' | 'medication' | 'alert';

export function MedicalDictionary({ onBack }: MedicalDictionaryProps) {
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedCategory, setSelectedCategory] = useState<Category>('all');

  // Filter dictionary entries
  const filteredEntries = Object.entries(medicalDictionary).filter(([term, data]) => {
      ...
      <div className="bg-gradient-to-r from-[#0066CC] to-[#004C99] text-white p-6 pb-8 sticky top-0 z-10">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-white/80">Learn about heart health terms</p>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <Input
            ...
            className="pl-11 h-12 bg-white border-0 shadow-md"
          />
        </div>
      </div>
      ...
      <div className="px-6 -mt-4">
        ...
        {filteredEntries.length === 0 ? (
          <Card className="p-8 bg-white border-2 border-dashed border-[#E0E0E0] text-center">
            ...
            <p className="text-[#666]">No terms found matching your search</p>
          </Card>
        ) : (
          <div className="space-y-3">
            {filteredEntries.map(([term, data]) => {
              ...
              return (
                <Card key={term} className="p-5 bg-white shadow-sm border-0">
                  <div className="space-y-3">
                    {/* Term Header */}
                    <div>
                    ...
                    <div className="p-4 bg-[#F8F9FA] rounded-lg">
                      <p className="text-[#0066CC] mb-2">What it means</p>
                      <p className="text-[#333]">{data.meaning}</p>
                    </div>

                    {/* Why It Matters */}
                    <div className="p-4 bg-[#F0F7FF] rounded-lg">
                      <p className="text-[#FF9500] mb-2">Why it matters</p>
                      <p className="text-[#333]">{data.whyItMatters}</p>
                    </div>
                  </div>
                </Card>
              );
